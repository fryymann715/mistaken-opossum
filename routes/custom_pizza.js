...
router.get( '/details/:pizza_id', ( request, response) => {
  const { pizza_id } = request.params
...
router.post( '/add', ( request, response ) => {
  const { crust_id } = request.body

  Promise.all([ CustomPizza.add( crust_id ) ])
  .then( data => { console.log(data[0].id)
    //response.send(data)
    response.redirect( `/custom_pizza/create/${data[0].id}/${crust_id}` )
  })
})

router.get( '/create/:pizza_id/:crust_id', (request, response ) => {
  const { pizza_id, crust_id } = request.params

  Promise.all([ CustomPizza.addCrust( pizza_id, crust_id ) ])
  .then( response.redirect( `/custom_pizza/details/${pizza_id}` ) )
})

router.post( '/add-topping/:pizza_id', ( request, response ) => {
  const { pizza_id } = request.params
  const { addTopping } = request.body

  Promise.all([ CustomPizza.addTopping( pizza_id, addTopping ) ])
  .then(
    response.redirect( `/custom_pizza/details/${pizza_id}` ))
})
